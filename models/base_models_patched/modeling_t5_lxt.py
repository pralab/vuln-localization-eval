...
from lxt.efficient.patches import stop_gradient,identity_rule_implicit, divide_gradient ### <------------------------------------------- LXT

logger = logging.get_logger(__name__)

class LXTDropout(nn.Dropout):
    # taken from lxt.efficient.patches -> dropout_forward
...
class T5LayerNorm(nn.Module):
    ...
    def forward(self, hidden_states): ### <------------------------------------------- LXT
        # taken from lxt.efficient.patches > rms_norm_forward 
        """
...
class T5DenseActDense(nn.Module):
    def __init__(self, config: T5Config):
        ...
        self.dropout = LXTDropout(config.dropout_rate) ### <------------------------------------------- LXT
        self.act = ACT2FN[config.dense_act_fn]

    def forward(self, hidden_states):
        hidden_states = self.wi(hidden_states)
        hidden_states = identity_rule_implicit(self.act, hidden_states) ### <------------------------------------------- LXT
...
class T5DenseGatedActDense(nn.Module):
    ...
    def forward(self, hidden_states):
        hidden_gelu = identity_rule_implicit(self.act, self.wi_0(hidden_states)) ### <------------------------------------------- LXT
        hidden_linear = self.wi_1(hidden_states)
        hidden_states = hidden_gelu * hidden_linear
        hidden_states = divide_gradient(hidden_states, 2) ### <------------------------------------------- LXT
        hidden_states = self.dropout(hidden_states)
...
class T5LayerFF(nn.Module):
    def __init__(self, config: T5Config):
        ...
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon) # TODO LXT, should I change it?
        self.dropout = LXTDropout(config.dropout_rate) ### <------------------------------------------- LXT
...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        scores = divide_gradient(scores, 2) ### <------------------------------------------- LXT
        ...
        scores = scores + position_bias_masked ### <------------------------------------------- LXT
        ...
        attn_output = torch.matmul(attn_weights, value_states)  # (batch_size, seq_length, dim)
        attn_product = divide_gradient(attn_output, 2) ### <------------------------------------------- LXT
        attn_output = unshape(attn_product)
...
class T5ClassificationHead(nn.Module):
    ...
    def __init__(self, config: T5Config):
        ...
        self.dropout = LXTDropout(p=config.classifier_dropout) ### <------------------------------------------- LXT
        self.out_proj = nn.Linear(config.d_model, config.num_labels)

    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.dense(hidden_states)
        hidden_states = identity_rule_implicit(torch.tanh, hidden_states) ### <------------------------------------------- LXT
        hidden_states = self.dropout(hidden_states)
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, T5LayerNorm):
            ...
            module,
            (T5Model, T5ForConditionalGenerationLXT, T5EncoderModel, T5ForQuestionAnswering),
        ):
...
class T5ForConditionalGenerationLXT(T5PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [
        "decoder.block.0.layer.1.EncDecAttention.relative_attention_bias.weight",
    ]
    ...
    def parallelize(self, device_map=None):
        warnings.warn(
            "`T5ForConditionalGenerationLXT.parallelize` is deprecated and will be removed in v5 of Transformers, you"
...
class T5ForTokenClassification(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        self.dropout = LXTDropout(config.classifier_dropout) ### <------------------------------------------- LXT
