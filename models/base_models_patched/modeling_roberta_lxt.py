...
from lxt.efficient.rules import identity_rule_implicit, divide_gradient, stop_gradient

logger = logging.get_logger(__name__)

class LXTLayerNorm(nn.LayerNorm):
    # taken from lxt.efficient.patches -> layer_norm_forward
...
class RobertaEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.LayerNorm = LXTLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = LXTDropout(config.hidden_dropout_prob)
...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        self.dropout = LXTDropout(config.attention_probs_dropout_prob)
    ...
    def forward(
        ...
        attention_scores = divide_gradient(attention_scores, 2) 
        ...
        context_layer = divide_gradient(context_layer, 2)
...
class RobertaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, LXTLayerNorm):
            module.bias.data.zero_()
...
class RobertaLMHead(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.layer_norm = LXTLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
    ...
    def forward(self, features, **kwargs):
        ...
        x = identity_rule_implicit(gelu, x) ### <------------------------------------------- LXT
        x = self.layer_norm(x)
...
class RobertaForSequenceClassification_LXT(RobertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
